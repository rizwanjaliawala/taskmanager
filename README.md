...
backed by Neon PostgreSQL, with Microsoft Graph email notifications and three scheduled
...
Express REST API  ──────►  Neon PostgreSQL   (Drizzle ORM, HTTP driver)
      │
      ├──►  Microsoft Graph  ──►  Microsoft 365  ──►  recipient mailbox
      │
...
**Requirements:** Node 20+, a Neon PostgreSQL database, and (for email) a Microsoft Entra
ID app registration.
...
| `MICROSOFT_TENANT_ID` | for email | Entra ID → App registration → Overview |
| `MICROSOFT_CLIENT_ID` | for email | Same page, *Application (client) ID* |
| `MICROSOFT_CLIENT_SECRET` | for email | Certificates & secrets → **Value**, not the Secret ID |
| `MICROSOFT_SENDER_EMAIL` | for email | The mailbox notifications are sent from |
| `EMAIL_FROM` | no | Display name on outgoing mail |
| `EMAIL_HOST/PORT/SECURE/USERNAME/PASSWORD` | no | Legacy SMTP fallback, used only when the `MICROSOFT_*` values are absent |
...
Four emails go out, all through Microsoft Graph:
...
Authentication is OAuth 2.0 **client credentials** — the app authenticates as itself, not
as a signed-in person, because cron jobs send with nobody present to consent. That
requires the **`Mail.Send` application permission with tenant admin consent**.

**This is not an email address and password.** The client secret is generated by Azure
for the application; `MICROSOFT_SENDER_EMAIL` is only *which mailbox to send from* and
has no password. Full setup: **[docs/EMAIL_SETUP.md](docs/EMAIL_SETUP.md)**.
...
src/lib/email/              transport, Graph client, renderer, five templates
...
**Not yet working: email delivery.** The Microsoft Graph transport is built and tested,
but the four `MICROSOFT_*` variables are not set in production because the Entra ID app
registration has not been created. Until then, notifications are recorded and marked
`failed` rather than sent — nothing is lost, and the retry sweep will flush the backlog
once credentials exist. See [docs/EMAIL_SETUP.md](docs/EMAIL_SETUP.md).
...
| `AADSTS7000215` | Wrong client secret, or the Secret **ID** was copied instead of the **Value** |
| `ErrorAccessDenied` on send | Admin consent not granted, or the permission was added as Delegated rather than Application |
| Email worked, then stopped | The client secret expired |
