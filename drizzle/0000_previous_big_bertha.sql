-- Hand-added (not generated by drizzle-kit, and absent from drizzle/meta/0000_snapshot.json's
-- "sequences": {}). tasks.ref defaults to 'UT-' || nextval('task_ref_seq'), and that expression
-- resolves the sequence name at DDL parse time, so the sequence must exist before CREATE TABLE
-- "tasks" runs. This statement must stay first in this migration file — do not remove or reorder
-- it, and re-add it manually if this migration is ever regenerated.
